	...
	class FSParser
		...
		def make_opt_parser
			@opt_parser = OptionParser.new do | opts |
				opts.on '-d', '--dirs', 'Make the empty directory structure' do @options << :dirs end
				...
				opts.on '-m', '--move', 'Scan the dirs, move files and make new dirs' do @options << :scan << :move end
